...
Each row in the dataset represents a **single mineral particle** analyzed by automated mineralogy (e.g., QEMSCAN or MLA). For each particle, the dataset contains:
...
3. **Survival variable mapping**: Map each class label to:
   ...
   - **E (Event)**: 1 = floated (recovered in concentrate), 0 = not floated (tailings, right-censored)
